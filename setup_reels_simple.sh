...
curl -s -X POST "$SUPABASE_URL/rest/v1/rpc/execute" \
  ...
  -H "Authorization: Bearer $SUPABASE_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "CREATE INDEX IF NOT EXISTS reels_user_id_idx ON public.reels (user_id)"}'
...
curl -s -X POST "$SUPABASE_URL/rest/v1/rpc/execute" \
  ...
  -H "Authorization: Bearer $SUPABASE_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "CREATE OR REPLACE FUNCTION increment_reel_likes_count(reel_id UUID) RETURNS VOID AS $$ BEGIN UPDATE public.reels SET likes_count = likes_count + 1, updated_at = now() WHERE id = reel_id; END; $$ LANGUAGE plpgsql SECURITY DEFINER"}'

curl -s -X POST "$SUPABASE_URL/rest/v1/rpc/execute" \
  -H "apikey: $SUPABASE_KEY" \
  -H "Authorization: Bearer $SUPABASE_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "CREATE OR REPLACE FUNCTION decrement_reel_likes_count(reel_id UUID) RETURNS VOID AS $$ BEGIN UPDATE public.reels SET likes_count = GREATEST(likes_count - 1, 0), updated_at = now() WHERE id = reel_id; END; $$ LANGUAGE plpgsql SECURITY DEFINER"}'
...
curl -s -X POST "$SUPABASE_URL/rest/v1/rpc/execute" \
  ...
  -H "Authorization: Bearer $SUPABASE_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "CREATE POLICY \"Reels are publicly accessible\" ON storage.objects FOR SELECT USING (bucket_id = '\''reels'\''"}'

curl -s -X POST "$SUPABASE_URL/rest/v1/rpc/execute" \
  -H "apikey: $SUPABASE_KEY" \
  -H "Authorization: Bearer $SUPABASE_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "CREATE POLICY \"Authenticated users can upload reels\" ON storage.objects FOR INSERT WITH CHECK (bucket_id = '\''reels'\'' AND auth.role() = '\''authenticated'\''"}'

curl -s -X POST "$SUPABASE_URL/rest/v1/rpc/execute" \
  -H "apikey: $SUPABASE_KEY" \
  -H "Authorization: Bearer $SUPABASE_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "CREATE POLICY \"Service role can manage reels\" ON storage.objects FOR ALL USING (bucket_id = '\''reels'\'' AND auth.role() = '\''service_role'\''"}'
